...
const NavBar = () => {
  ...
  return (
    <div className='sticky top-0 z-10 w-screen h-12 max-[500px]:flex-col max-[500px]:h-24 pad:h-16 res:h-16 bg-blue-950 flex justify-between items-center py-2 px-5 res:px-9'>

      {/* logo */}
      <Link to="/">
        <div className='rubik-head max-[500px]:text-4xl pad:text-2xl res:text-4xl flex'>
      ...
      {/* search */}
      <form onSubmit={handleSubmit} className='h-8 max-[500px]:hidden res:h-10 w-60 lg:w-96 flex  items-center bg-blue-200 rounded-md'>
      ...
      <div className='flex items-center'>
        ...
        <form onSubmit={handleSubmit} className='h-8 pad:hidden w-40 mx-5 place-self-start  flex items-center bg-blue-200 rounded-md'>

          <input value={searchTerm} onChange={(e) => { setsearchTerm(e.target.value) }} className='rounded pl-5 h-8 w-32 lg:w-96 bg-blue-200 text-blue-950 placeholder:text-blue-950 placeholder:italic text-justify focus:outline-none' placeholder='Search...' type='text' />
